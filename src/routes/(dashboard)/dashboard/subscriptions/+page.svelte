<script lang="ts">
	...
	const subscriptionsQuery = useQuery(api.subscriptions.list, { activeOnly: false });
...
<div class="space-y-6">
	...
	<div class="flex items-center justify-between">
		...
		<span class="text-sm text-secondary">
			{#if subscriptionsQuery.data}
				{subscriptionsQuery.data.length} total
			{/if}
		</span>
	</div>
	...
	{:else if subscriptionsQuery.data && subscriptionsQuery.data.length > 0}
		<div class="space-y-3">
			{#each subscriptionsQuery.data as subscription}
				{@const daysUntil = getDaysUntil(subscription.nextBillingDate)}
				<button
					onclick={() => goto(`/dashboard/subscriptions/${subscription._id}`)}
					class="w-full bg-surface hover:bg-surface-elevated rounded-xl p-4 transition-colors text-left"
				>
					<div class="flex items-center gap-4">
						<!-- Icon -->
						<div class="w-12 h-12 bg-zinc-800 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0">
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center gap-2">
								<h3 class="font-medium text-white truncate">{subscription.name}</h3>
								{#if !subscription.isActive}
									<span class="px-2 py-0.5 text-xs bg-zinc-700 text-zinc-400 rounded">Inactive</span>
								{/if}
							</div>
						...
						<div class="text-right flex-shrink-0">
							<p class="font-medium text-white">
								...
								<span class="text-secondary text-sm">{getBillingLabel(subscription.billingCycle)}</span>
							</p>
	...
	{:else}
		<div class="text-center py-12">
			<div class="w-16 h-16 mx-auto mb-4 bg-surface rounded-full flex items-center justify-center">
				<svg class="w-8 h-8 text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
				</svg>
			</div>
			<h3 class="text-lg font-medium text-white mb-2">No subscriptions yet</h3>
			<p class="text-secondary mb-6">Add your first subscription to start tracking your spending.</p>
			<button
