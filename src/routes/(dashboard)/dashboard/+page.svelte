<script lang="ts">
	...
	import {
		...
		getTotalMonthlyCost as getGuestTotalCost,
		getUpcomingSubscriptions as getGuestUpcoming,
	...
	// Guest mode derived data
	const guestSubscriptions = $derived(getGuestSubscriptions());
	const guestUpcoming = $derived(getGuestUpcoming(7));
	const guestTotalCost = $derived(getGuestTotalCost());
	const guestActiveSubscriptions = $derived(
		guestSubscriptions.filter((s) => s.isActive),
	);

	// Unified data accessors based on mode
	...
	const upcomingData = $derived(
		isGuestMode
			? guestUpcoming.map((sub) => ({
					_id: sub.localId,
					name: sub.name,
					cost: sub.cost,
	...
	const totalCostData = $derived(
		isGuestMode ? guestTotalCost : (totalCostQuery.data ?? {}),
	);

	const activeSubscriptionsCount = $derived(
		isGuestMode
			? guestActiveSubscriptions.length
			: (subscriptionsQuery.data?.length ?? 0),
	);

	const subscriptionsData = $derived(
		isGuestMode
			? guestActiveSubscriptions.map((sub) => ({
					_id: sub.localId,
					name: sub.name,
					cost: sub.cost,
					currency: sub.currency,
	...
	const primaryTotal = $derived(() => {
		const currencies = Object.entries(totalCostData);
...
<div class="space-y-6">
	...
	<div>
		<h1 class="text-2xl font-bold text-white">
			{#if !isGuestMode && userQuery.data?.name}
	...
	<div class="space-y-3">
		<div class="flex items-center justify-between">
			...
			{#if upcomingData && upcomingData.length > 0}
				<span class="text-sm text-secondary"
					>{upcomingData.length} renewal{upcomingData.length !== 1
						? "s"
						: ""}</span
				>
			{/if}
		...
		{:else if upcomingData && upcomingData.length > 0}
			<div class="space-y-2">
				{#each upcomingData as sub}
					{@const daysUntil = getDaysUntil(sub.nextBillingDate)}
					<button
						onclick={() => handleSubscriptionClick(sub._id)}
						class="w-full p-4 bg-surface hover:bg-surface-elevated rounded-xl transition-colors text-left"
					>
						<div class="flex items-center gap-3">
							<div
								class="w-10 h-10 bg-zinc-800 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0"
							>
								{#if sub.website}
									<img
							...
							<div class="text-right flex-shrink-0">
								...
								{#if daysUntil <= 3}
									<p class="text-xs text-accent-coral">
	...
	{#if wasteAlerts().length > 0}
		<div class="space-y-3">
			<div class="flex items-center gap-2">
				...
				<span
					class="px-2 py-0.5 text-xs bg-accent-coral/20 text-accent-coral rounded-full"
				>
					{wasteAlerts().length}
				</span>
			</div>
			...
			<div class="space-y-2">
				{#each wasteAlerts() as alert}
					<button
						onclick={() => handleSubscriptionClick(alert._id)}
						...
						<div class="flex items-center gap-3">
							...
							<div class="flex-1 min-w-0">
								...
								<p class="text-sm text-accent-coral">
									{alert.unusedSlots} unused slot{alert.unusedSlots !== 1
										? "s"
										: ""}
								</p>
							</div>
